...
export class AuthService {
  ...
  sendVerificationEmail(email: string): Observable<string> {
    return this.http.get(USER_VERIFY_EMAIL_URL + email, { responseType: 'text' });
  }
  ...
  searchRegisteredUsers(searchTerm: string) {
    return this.http.get<User[]>(SEARCH_REGISTERED_USER_URL + searchTerm);
  }

  searchPendingUsers(searchTerm: string) {
    return this.http.get<User[]>(GET_PENDING_USER_URL + searchTerm);
  }
